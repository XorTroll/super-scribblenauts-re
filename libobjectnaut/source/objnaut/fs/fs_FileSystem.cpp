...
namespace objnaut::fs {
    ...
    Result FileSystem::ReadFrom(twl::fs::File &rf_1i, twl::fs::File &rf_1p) {
        this->files.clear();

        u32 file_count;
        TWL_R_TRY(rf_1i.Read(file_count));
        ...
        TWL_R_TRY(rf_1i.ReadBuffer(file_offsets, (file_count + 1) * sizeof(u32)));
        ...
        for(u32 i = 0; i < file_count; i++) {
            ...
            if(file_size > 0) {
                TWL_R_TRY(rf_1p.SetAbsoluteOffset(file_offset));

                auto file_buf = new u8[file_size]();
                TWL_R_TRY(rf_1p.ReadBuffer(file_buf, file_size));
    ...
    Result FileSystem::WriteTo(twl::fs::File &wf_1i, twl::fs::File &wf_1p) {
        TWL_R_TRY(wf_1i.Write<u32>(this->files.size()));
        
        size_t cur_offset;
        u32 i = 0;
        for(auto &file: this->files) {
            TWL_R_TRY(wf_1p.GetOffset(cur_offset));
            TWL_R_TRY(wf_1i.Write<u32>(cur_offset));
            ...
            if(file.buf.IsValid()) {
                if(file.was_compressed) {
                    ...
                    TWL_R_TRY(wf_1p.WriteBuffer(comp_file_buf, comp_file_size));
                    TWL_R_TRY(wf_1p.Write<u8>(0x01));

                    delete[] comp_file_buf;
                }
                else {
                    TWL_R_TRY(wf_1p.WriteBuffer(file.buf.GetBuffer(), file.buf.GetBufferSize()));
                    TWL_R_TRY(wf_1p.Write<u8>(0x00));
                }
            }
